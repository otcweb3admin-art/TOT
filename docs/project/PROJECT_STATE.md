...
- **下一步 P2-017（待用户定）**：可选 ① 角色按需细分（collector/executor/… 差异化可见·可写，或第 7 角色 `outsource` 定夺）② 某 TB 深化 / ③ MVS·Metric·Experience / ④ action-helper 轻量重构（审计 #5，第二轮）/ ⑤ 经营承接能力深化（履约/组织 risk 在工作台高亮、纳入诊断维度）/ ⑥ smoke 扩展（HTTP 层/CI）。注：`admin@tot.local` 现仍为 `operator` 角色（仅见自有商家）；若需全见，需人工将其置为 `admin`（角色=业务决策，AI 不拍板）。仍守 AI 不拍板 / 人工审核。
