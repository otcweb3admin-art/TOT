...
- 2026-06-03：**P1 登录运行时端到端验证通过**（浏览器实跑 `login→dashboard→logout→守卫`；DAL 首登建 `UserProfile{operator,active}`）。两坑记牢：① **DB 直建 `auth.users` 测试用户**必须把 GoTrue 的 `confirmation_token/recovery_token/email_change/email_change_token_*/reauthentication_token/phone_change*` 等列由 `NULL` 补成空串、`email_change_confirm_status` 补 0，否则登录报 `500 Database error querying schema`（GoTrue 按"非空字符串"扫描这些列）；② Supabase **新版 API key**（`sb_publishable_…`/`sb_secret_…`，非 `eyJ…` JWT）`supabase-js`/`@supabase/ssr` 均兼容：anon=publishable、service_role=secret。
